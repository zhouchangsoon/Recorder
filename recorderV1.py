...
----------------------------------------------------------------------------------~(*)
'''
import cv2  #addByChangsoon
import socket #addByChangsoon
import ImageEnhance  #addByChangsoon use to enhance the contrast of an image
from pytesseract import * #addByChangsoon use to OCR
import PIL import Image #addByChangsoon use to read or write Image
import codecs #addByChangsoon use to write unicode word
...
'''
            ...
            if len(self.gaze_point_x) > 0 and self.frame_count == 200:
                if (self.gaze_point_x[-1] > 0 and self.gaze_point_x[-1] < 1 and self.gaze_point_y[-1] > 0 and self.gaze_point_y[-1] < 1):
                    height = (1 - self.gaze_point_y[-1]) * 720
                    height = int(height)
                    image = frame.image
                    if (height < 100):
                        image = frame.image[0:height+200, 300:1000]
                    elif (height > 520):
                        image = frame.image[height-100:720, 300:1000]
                    else:
                        image = frame.image[height-100:height+200, 300:1000]
                    cv2.imwrite('/home/eyetracking/pupil/'+"frame%d.tiff" % self.frame_count,image)
                    image=Image.open('/home/eyetracking/pupil/'+"frame%d.tiff" %self.frame_count)
